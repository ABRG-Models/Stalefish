/*
 * NB: This is a HACKED version of sfview for generating a paper figure. Use sfview!
 *
 ...
 * entirely procedurally. The viewing methods used here could potentially be integrated
 * into the main stalefish application, although some of hte features here allow for the
    ...
    try {
...

        {
            ...
            for (int i = 1; i<=nf; ++i) {
                ...
                std::cout << "Size of linbins vs frameQuads_lmaligned: " << linbins.size() << "/" << frameQuads_lmaligned.size() << "\n";
                for (unsigned int ii = 0; ii < frameQuads_lmaligned.size()-1; ++ii) {
                    ...
                    frameQuads_lmaligned[ii][0] = pt_trans[0];
                    frameQuads_lmaligned[ii][1] = pt_trans[1];
                    frameQuads_lmaligned[ii][2] = pt_trans[2];

                    frameQuads_lmaligned[ii][3] = pt_trans[4];
                    frameQuads_lmaligned[ii][4] = pt_trans[5];
                    frameQuads_lmaligned[ii][5] = pt_trans[6];

                    frameQuads_lmaligned[ii][6] = pt_trans[8];
                    frameQuads_lmaligned[ii][7] = pt_trans[9];
                    frameQuads_lmaligned[ii][8] = pt_trans[10];

                    frameQuads_lmaligned[ii][9] = pt_trans[12];
                    frameQuads_lmaligned[ii][10] = pt_trans[13];
                    frameQuads_lmaligned[ii][11] = pt_trans[14];
                }
                frameQuads_lmaligned[frameQuads_lmaligned.size()-1] = frameQuads_lmaligned[frameQuads_lmaligned.size()-2];
                // END HACKED
            ...
            for (int i = 1; i<=nf; ++i) {
                ...
                try {
                    if (autoscale_per_slice) {
                        ...
                        str = frameName+"/signal/postproc/boxes/means";
                        d.read_contained_vals (str.c_str(), frameMeansF);
                        // The morph::Scale object scale with autoscale the whole thing.
                        scale.do_autoscale = true;
                    }
                } catch (const exception& ee) {

/*
 * Stalefish - extension to PointRowsVis class from morphologica
...
namespace morph {
    ...
    /*!
     * Hacked PointRowsVisual with raw colour
     */
    template <typename Flt>
    class PointRowsVisExt : public VisualDataModel<Flt>
    {
    public:
        PointRowsVisExt(GLuint sp,
        ...
        //! Perhaps the colours are set by the client code and passed in? Here's where they go.
        std::vector<std::array<float, 3>> dataRawColour;

        PointRowsVisExt(GLuint sp,
                        std::vector<Vector<float,3>>* _pointrows,
                        const Vector<float, 3> _offset,
                        const std::vector<std::array<float, 3>>& _colourdata,
                        const Scale<Flt>& cscale,
                        ColourMapType _cmt,
                        const float _hue = 0.0f)
        {
            ...
            this->dataCoords = _pointrows;
            this->dataRawColour = _colourdata; // data copy
        ...
        void initializeVertices()
        {
            ...
            while (r2 != prlen) { // While through all 'rows' - pairs of pointrows
                ...
                if (this->dataRawColour.empty()) {
                    ...
                    this->vertex_push (this->dataRawColour[r1], this->vertexColors);
                }
                ...
                if (this->dataRawColour.empty()) {
                    ...
                    this->vertex_push (this->dataRawColour[r2], this->vertexColors);
                }
                //this->vertex_push (0.0f, 0.0f, 1.0f, this->vertexNormals);
                this->indices.push_back (ib);
                ib++;

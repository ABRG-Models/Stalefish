/*
 * Stalefish - extension to QuadsVisual class from morphologica (for raw colour)
...
namespace morph {
    ...
    class QuadsVisualExt : public VisualDataModel<Flt>
    {
    public:
        QuadsVisualExt (GLuint sp,
        ...
        ~QuadsVisualExt() { delete this->dataCoords; }
        ...
        void initializeVertices()
        {
            ...
            if (nquads != ndata) {
                std::cout << "nquads (" << nquads << ") != ndata ("<<ndata<<"), return." << std::endl;
                return;
            }
            ...
            for (unsigned int qi = 0; qi < nquads; ++qi) {
                ...
                if (this->dataRawColour.empty()) {
                    clr = this->cm.convert(dcopy[qi]);
                } else {
                    clr = this->dataRawColour[qi];
                }

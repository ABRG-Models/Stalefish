...
with h5py.File (filename, 'r') as f:
    ...
    # Data is x,y,means. Reshape to make tricontours
    x_tri = x
    x_tri = np.reshape (x_tri, np.size(x_tri))
    y_tri = y
    y_tri = np.reshape (y_tri, np.size(y_tri))
    print('{0}'.format(np.shape(y)))
    print('{0}'.format(np.shape(y_tri)))
    means_tri = means
    means_tri = np.reshape (means_tri, np.size(means_tri))
    means_autoscaled_tri = np.reshape (means_autoscaled, np.size(means_autoscaled))
    ...
    if n == 3 or n == 1:
        ...
        plt.tricontour (x_tri, y_tri, means_tri, levels=[0.5], linewidths=0.2, colors="black")
    ...
    if n == 3 or n == 2:
        ...
        #
        plt.tricontour (x_tri, y_tri, means_autoscaled_tri, levels=[0.8], linewidths=0.2, colors="black")
        #    plt.tight_layout()
        #    plt.show()
        #    time.sleep(2)

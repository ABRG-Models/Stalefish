 ...
 * **global_landmarks**: The frame locations of the user-supplied global landmarks.
 ...
 * **output_map**: This structure is added by sfview in a post-processing
   step. It holds the post-transformed 2D maps.
...
Global landmark locations are stored within the Frames, but there is an array stored in the root of the HDF5 file structure.

**/global_landmarks** is an matrix of n columns, where n is the number of global landmarks added to the project. The first row records the frame number for each global landmark and the second row gives the index within the frame. This index permits more than one global landmark to be added to any single frame. The order in which the global landmarks are listed in **/global_landmarks** is important, and matches the order in which the user added them to the project.

### output_map

Linearly transformed maps, including resampled maps.
